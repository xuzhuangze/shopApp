<template>
	<view class="container">
		...
		<view class="detailImage">
			<image :src="baseUrl +  goodsdetail.img">
		</view>
		<!-- 详情基本信息 -->
		<view class="detailInfo">
			<view class="detailInfoName">
				<label>{{goodsdetail.goodsname}}</label>
			</view>
			<view class="detailInfoPrice">
				<label style="color: red;">￥ {{goodsdetail.price}}</label>
		...
		<view class="changeInfo">
			...
			<view class="changeState">
				...
				<view class="productGuige">
					<label></label>
					<view class="bg">{{goodsdetail.specsattr}}</view>
		...
		methods: {
			...
			getdetail(id) {
				...
				this.http({
					...
					// console.log(res);
					this.goodsdetail = res.data.list ? res.data.list[0] : {};
				})
			}
